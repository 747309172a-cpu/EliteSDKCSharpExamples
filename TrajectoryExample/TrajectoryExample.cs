...
internal static class TrajectoryExample
{
    ...
    private const string DefaultLocalIp = "";
    private const bool DefaultHeadless = true;
    private const string DefaultScriptFile = "resource/external_control.script";
    ...
    private sealed class TrajectoryControl : IDisposable
    {
        ...
        private bool moveTrajectory(IReadOnlyList<double[]> targetPoints, float pointTime, float blendRadius, bool isCartesian, float speed, float acceleration)
        {
            ...
            foreach (var point in targetPoints)
            {
                ...
                {
                    Console.WriteLine("[ERROR] Failed to send NOOP.");
                    return false;
                }
            }
            ...
            Console.WriteLine($"[INFO] Trajectory motion completed: {result}");
            if (!_driver.writeIdle(0))
            {
                Console.WriteLine("[ERROR] Failed to write idle.");
                return false;
            }
    ...
    internal static void Run(string[] args)
    {
        ...
        var localIp = DefaultLocalIp;
        var headless = DefaultHeadless;
        var scriptFile = DefaultScriptFile;
        ...
        for (var i = 2; i < args.Length; i++)
        {
            ...
            else if (args[i] == "--headless" || args[i] == "--use-headless-mode")
            {
                headless = true;
            }
        ...
        try
        {
            ...
            {
                Console.WriteLine($"[FATAL] Can't connect RTSI server: {config.RobotIp}");
                return;
            }
